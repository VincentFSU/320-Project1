...
public class Buffer
{
    ...
    public void Concat(byte[] newData, int numOfBytes = -1)
    {
        ...
        if (numOfBytes < 0 || numOfBytes > newData.Length)
        {
            numOfBytes = newData.Length;
        }
    ...
    public override string ToString()
    {
        ...
        {
            sb.Append(" ");
            sb.Append(b.ToString("x2")); //format to hex
        }
    ...
    public ushort ReadUInt16LE(int offset = 0)
    {
        ...
        return (ushort)((b << 8) | a); // shift b 8 bits left and OR | it with a
    }
    ...
    public ushort ReadUInt16BE(int offset = 0)
    {
        ...
        return (ushort)((a << 8) | b); // shift a 8 bits left and OR | it with b
    }
    ...
    public short ReadInt16LE(int offset = 0)
    {
        ...
        return (short)((b << 8) | a); // shift b 8 bits left and OR | it with a
    }
    ...
    public short ReadInt16BE(int offset = 0)
    {
        ...
        return (short)((a << 8) | b); // shift a 8 bits left and OR | it with b
    }
    ...
    {
        WriteByte((byte)val, offset); // pass in the first 8 bits of the byte
        WriteByte((byte)(val >> 8), offset + 1); // pass in the second 8 bits of the byte
    }

    // write unsigned 16-bit BE
    public void WriteUInt16BE(ushort val, int offset = 0)
    {
        WriteByte((byte)(val >> 8), offset); // pass in the second 8 bits of the byte in to the first position
        WriteByte((byte)val, offset + 1); // pass in the first 8 bits of the byte in to the second positon
    }
    ...
    // write unsigned 32-bit LE
    public void WriteUInt32LE(uint val, int offset = 0)
    {
        WriteByte((byte)(val >> 00), offset + 0); // pass in the first 8 bits of the byte
        WriteByte((byte)(val >> 08), offset + 1); // pass in the second 8 bits of the byte
        WriteByte((byte)(val >> 16), offset + 2); // pass in the third 8 bits of the byte
        WriteByte((byte)(val >> 24), offset + 3); // pass in the fourth 8 bits of the byte
    }

    // write unsigned 32-bit BE
    public void WriteUInt32BE(uint val, int offset = 0)
    {
        WriteByte((byte)(val >> 24), offset + 0); // pass in the fourth 8 bits of the byte
        WriteByte((byte)(val >> 16), offset + 1); // pass in the third 8 bits of the byte
        WriteByte((byte)(val >> 08), offset + 2); // pass in the second 8 bits of the byte
        WriteByte((byte)(val >> 00), offset + 3); // pass in the first 8 bits of the byte
    }

    // write signed 32-bit LE
    ...
    {
        // BitConverter reads BE so we flip the order around and store it in a temp array:
        byte[] temp = new byte[]
        {
            ReadByte(offset + 3),
            ReadByte(offset + 2),
